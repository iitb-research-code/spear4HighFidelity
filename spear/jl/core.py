...
class JL:
	...
	def fit_and_predict_proba(self, path_L, path_U, path_V, path_T,train,dev,test, loss_func_mask, batch_size, lr_fm, lr_gm, use_accuracy_score, path_log = None, return_gm = False, n_epochs = 100, start_len = 7,\
	 stop_len = 10, is_qt = True, is_qc = True, qt = 0.9, qc = 0.85, metric_avg = 'binary'):
		...
		#############################################################################################################################################
		#Algo starting
		optimizer_fm = torch.optim.Adam(self.feature_model.parameters(), lr = 5e-5)
		...
		with tqdm(total=n_epochs_) as pbar:
			...
			for epoch in range(n_epochs_):
				...
				if use_accuracy_score:
					...
					fm_test_acc = f1(out_label_list, preds_list, average = metric_avg)
				fm_test_prec = precision_score(out_label_list, preds_list, average = metric_avg)
				fm_test_recall = rec(out_label_list, preds_list, average = metric_avg)
				...
				if use_accuracy_score:
					...
					fm_valid_acc = f1(out_label_listz, preds_listz, average = metric_avg)
					print("fm_validation",fm_valid_acc)
				(self.feature_model).train()
